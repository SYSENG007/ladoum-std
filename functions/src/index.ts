 ...
 */
export const sendInvitationEmail = functions.https.onCall(async (data: SendInvitationEmailData, context) => {
    // Verify user is authenticated
    if (!context.auth) {
        throw new functions.https.HttpsError(
            'unauthenticated',
            'L\'utilisateur doit être authentifié pour envoyer des invitations.'
        );
    }
    ...
    if (!email || !code || !inviterName) {
        throw new functions.https.HttpsError(
            'invalid-argument',
            'Email, code et nom de l\'inviteur sont requis.'
        );
    }
