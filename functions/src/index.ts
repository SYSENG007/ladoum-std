...
async function getFarmOwner(farmId: string): Promise<string | null> {
    const farmDoc = await db.collection('farms').doc(farmId).get();
    if (!farmDoc.exists) return null;
    return (farmDoc.data() as Farm).ownerId;
}
...
export const checkVaccinationReminders = functions
    ...
    .onRun(async (_context) => {
        ...
        for (const doc of animalsSnapshot.docs) {
            const animal = { id: doc.id, ...doc.data() } as Animal;

            if (!animal.healthRecords) continue;
                ...
                if (record.type === 'Vaccination' && record.nextDue) {
                    ...
                    if (nextDueDate <= in7Days && nextDueDate >= today) {
                        ...
                        await createNotification(
                            ...
                            `${animal.name} (${animal.tagId}) doit être vacciné(e) le ${formatDate(nextDueDate)}`,
                            {
...
export const checkOverdueTasks = functions
    ...
    .onRun(async (_context) => {
        ...
        // Group by farm
        const tasksByFarm: Map<string, Task[]> = new Map();
        ...
        for (const [farmId, tasks] of tasksByFarm.entries()) {
            ...
            if (tasks.length === 1) {
                await createNotification(
                    ...
                    'task_overdue',
                    'Tâche en retard',
                    `La tâche "${tasks[0].title}" est en retard`,
                    {
                        priority: 'high',
                        actionUrl: '/tasks',
                ...
                await createNotification(
                    ownerId,
                    'task_overdue',
                    `${tasks.length} tâches en retard`,
                    `Vous avez ${tasks.length} tâches en retard à traiter`,
                    {
                        priority: 'high',
                        actionUrl: '/tasks',
                        actionLabel: 'Voir les tâches',
...
export const checkLowStock = functions
    ...
    .onRun(async (_context) => {
        ...
        // Group by farm
        const itemsByFarm: Map<string, InventoryItem[]> = new Map();
        ...
        for (const [farmId, items] of itemsByFarm.entries()) {
            ...
            if (items.length === 1) {
                ...
                await createNotification(
                    ...
                    'low_stock',
                    'Stock faible',
                    `${item.name}: seulement ${item.quantity} ${item.unit} restant(s)`,
                    {
                        priority: 'high',
                        actionUrl: '/inventory',
                ...
                await createNotification(
                    ownerId,
                    'low_stock',
                    `${items.length} articles en stock faible`,
                    `${items.map(i => i.name).join(', ')} sont en dessous du seuil minimum`,
                    {
                        priority: 'high',
                        actionUrl: '/inventory',
                        actionLabel: "Voir l'inventaire",
    ...
    .onRun(async (_context) => {
        ...
        for (const farmDoc of farmsSnapshot.docs) {
            ...
            await createNotification(
                farm.ownerId,
                'system',
                `Rapport de ${monthName}`,
                `Revenus: ${totalIncome.toLocaleString()} FCFA | Dépenses: ${totalExpenses.toLocaleString()} FCFA | Bénéfice: ${(totalIncome - totalExpenses).toLocaleString()} FCFA`,
                {
                    priority: 'low',
...
export const checkHeatPredictions = functions
    .runWith({ timeoutSeconds: 120, memory: '256MB' })
    .pubsub
    .schedule('0 7 * * *')
    .timeZone('Africa/Dakar')
    .onRun(async (_context) => {
        ...
        for (const doc of animalsSnapshot.docs) {
            const animal = { id: doc.id, ...doc.data() } as Animal & { reproductionRecords?: any[] };

            if (!animal.reproductionRecords || animal.reproductionRecords.length === 0) continue;
            ...
            if (nextHeatDate >= today && nextHeatDate <= in3Days) {
                ...
                await createNotification(
                    ...
                    'heat_prediction',
                    'Chaleurs prévues',
                    `${animal.name} devrait entrer en chaleurs vers le ${formatDate(nextHeatDate)}`,
                    {
                        priority: 'medium',
                        actionUrl: `/herd/${animal.id}`,
                        actionLabel: "Voir l'animal",
